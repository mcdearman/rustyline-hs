...
import Rustyline.Completion (Completer (..), Candidate (..))
import Rustyline.Hint (Hinter (..), Hint (..))
...
import System.IO (hFlush, stdout, stdin, putStr, hSetEncoding, utf8)
...
readlineWithInitial
  :: Helper h => Editor h -> String -> (String, String)
...
data EState = EState
  { sBuf     :: !LineBuffer
  , sHistIdx :: !(Maybe Int)  -- ^ Nothing = fresh line; Just i = viewing entry i.
  , sStash   :: !String       -- ^ Saved fresh line while browsing history.
  , sCompl   :: !(Maybe Compl)
  , sRow     :: !Int          -- ^ Cursor row offset from block top, last frame.
  }
...
dispatch
  :: Helper h => Editor h -> String -> EState -> Cmd
  -> IO (Either ReadlineError String)
dispatch ed prompt st cmd =
...
histPrev ed prompt st = do
  ...
  if n == 0
    then beep >> loop ed prompt (clearCompl st)
      ...
      loop ed prompt st { sBuf = fromString entry
...
histNext ed prompt st = do
  ...
  case sHistIdx st of
    ...
    Just i
      | i + 1 >= n -> loop ed prompt st { sBuf = fromString (sStash st)
                                        , sHistIdx = Nothing
                                        , sCompl = Nothing }
      | otherwise  -> loop ed prompt st { sBuf = fromString (entries !! (i + 1))
                                        , sHistIdx = Just (i + 1)
                                        , sCompl = Nothing }
      ...
      case sCompl st of
        ...
        _ -> do
          ...
          case reps of
            ...
            [r] -> let line' = take start line ++ r ++ drop pos line
                   in loop ed prompt st { sBuf = setLine line' (start + length r)
                                        , sCompl = Nothing }
            _   -> case completionType (edConfig ed) of
              Circular ->
                let cand  = head reps
                    line' = take start line ++ cand ++ drop pos line
                in loop ed prompt st { sBuf = setLine line' (start + length cand)
...
render ed prompt st = do
  ...
  renderBlock Frame
    { fPrompt = styledPrompt, fPromptLen = plen
    , fStyled = styledLine, fRaw = line, fHint = hintStr
    , fCursor = pos, fOldRow = sRow st }
